...
public class CommentServiceImpl implements CommentService {
    ...
    public List<String> getComments(String video_id) {
        ...
        queryWrapper.eq("video_id", video_id)
                   .select("comment_id", "username", "video_id", "comment", "comment_time", "zan");

        return commentMapper.selectList(queryWrapper).stream()
                .map(comment -> String.format(
                    "commentID: %s, username: %s, video_id: %s, comment: %s, comment_time: %s, zan: %s",
    ...
    public List<String> getAllComments() {
        QueryWrapper<Comment> queryWrapper = new QueryWrapper<>();
        queryWrapper.select("comment_id", "username", "video_id", "comment", "comment_time", "zan");
